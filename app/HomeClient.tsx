...
export default function HomeClient({ featuredProjects }: { featuredProjects: any[] }) {
  ...
  const experienceData = [
    {
      ...
      logo: "/logos/logo-timephoria.png"
    },
    {
      ...
      <main className="max-w-7xl mx-auto px-6 md:px-12 pb-24 space-y-20">
        ...
        <section className="relative pt-32 md:pt-48 pb-24 min-h-[85vh] flex flex-col justify-center border border-transparent overflow-hidden rounded-b-[3rem]">
          ...
          <motion.div variants={textRevealContainer} initial="hidden" animate="visible" className="relative z-10 flex flex-col items-start w-full max-w-4xl pointer-events-none">
            ...
            {/* NEW: Status Availability */}
            <div className="mt-6 inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-neutral-100 dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800">
            ...
            <motion.div variants={textRevealItem} className="mt-12 flex flex-col items-start gap-8 pointer-events-auto w-full">
              <div className="flex flex-col sm:flex-row items-center gap-4 w-full sm:w-auto">
                ...
                <a href="/documents/cv-aditya.pdf" target="_blank" rel="noopener noreferrer" download="documents/cv-aditya.pdf" className="inline-flex items-center gap-2 px-6 py-3.5 bg-neutral-900 dark:bg-neutral-100 text-white dark:text-neutral-950 font-medium rounded-full hover:scale-105 transition-transform duration-300 shadow-md">
              ...
              <div className="flex flex-wrap items-center gap-6 sm:gap-12 pt-6 border-t border-neutral-200/50 dark:border-neutral-800/50 w-full max-w-2xl">
                <div className="flex flex-col">
                  <span className="text-2xl md:text-3xl font-bold text-neutral-900 dark:text-white tracking-tight">7+</span>
                  <span className="text-xs uppercase tracking-wider font-medium text-neutral-500">{t("stat.brandsProjects" as any)}</span>
                </div>
                <div className="flex flex-col">
                  <span className="text-2xl md:text-3xl font-bold text-neutral-900 dark:text-white tracking-tight">30M+</span>
                  <span className="text-xs uppercase tracking-wider font-medium text-neutral-500">{t("stat.minVideo" as any)}</span>
                </div>
                <div className="flex flex-col">
                  <span className="text-2xl md:text-3xl font-bold text-neutral-900 dark:text-white tracking-tight">1,000+</span>
                  <span className="text-xs uppercase tracking-wider font-medium text-neutral-500">{t("stat.yoy" as any)}</span>
                </div>
                <div className="flex flex-col">
                  <span className="text-2xl md:text-3xl font-bold text-neutral-900 dark:text-white tracking-tight">310.9%</span>
        ...
        {/* SELECTED WORKS (DYNAMIC FROM SANITY) */}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {featuredProjects && featuredProjects.length > 0 ? (
              featuredProjects.map((project: any, index: number) => (
                <motion.div key={project.id || index} variants={fadeUp} className="group">
                  <Link href={`/work/${project.id}`} className="block space-y-4">
                    <div className="aspect-[4/5] bg-neutral-200 dark:bg-neutral-800 border border-neutral-200/50 dark:border-neutral-800/50 rounded-3xl overflow-hidden relative shadow-lg transition-transform duration-500 group-hover:-translate-y-2">
                      {project.thumb ? (
                        <Image src={project.thumb} alt={project.title?.[language] || project.title?.en || "Project"} fill className="object-cover" />
                      ) : (
                        <div className="w-full h-full placeholder-thumbnail"></div>
                      )}
                      
                      {project.category && (
                        <div className="absolute top-4 left-4 flex gap-2">
                          <span className="px-3 py-1 bg-black/60 text-white text-xs font-medium rounded-full backdrop-blur-md">
                            {project.category}
                          </span>
                        </div>
                      )}
                    ...
                    <div>
                      ...
                      <p className="text-sm text-neutral-500 line-clamp-1">{project.role || "-"}</p>
                    </div>
                  </Link>
                </motion.div>
              ))
            ) : (
              <div className="col-span-3 py-12 text-center border-2 border-dashed border-neutral-200 dark:border-neutral-800 rounded-3xl">
                <p className="text-neutral-500">
                  {language === 'en' 
                    ? "No featured projects yet. Go to Sanity Studio and toggle 'Tampilkan di Homepage?' on your projects."
                    : "Belum ada proyek unggulan. Buka Sanity Studio dan centang 'Tampilkan di Homepage?' pada proyekmu."}
                </p>
              </div>
            )}
          </div>
        ...
        <motion.section variants={staggerContainer} initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} className="space-y-8 relative z-10 pt-16">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <motion.div variants={fadeUp} className="p-5 bg-white/60 dark:bg-neutral-900/40 backdrop-blur-md border border-neutral-200/50 dark:border-neutral-800/50 rounded-2xl flex flex-col items-start gap-3">
              ...
              <div>
                <div className="font-semibold text-neutral-900 dark:text-neutral-100">{t("achieve.1.title" as any)}</div>
                <div className="text-xs leading-relaxed text-neutral-500 mt-1">{t("achieve.1.desc" as any)}</div>
              </div>
            </motion.div>
            ...
            <motion.div variants={fadeUp} className="p-5 bg-white/60 dark:bg-neutral-900/40 backdrop-blur-md border border-neutral-200/50 dark:border-neutral-800/50 rounded-2xl flex flex-col items-start gap-3">
              ...
              <div>
                <div className="font-semibold text-neutral-900 dark:text-neutral-100">{t("achieve.2.title" as any)}</div>
                <div className="text-xs leading-relaxed text-neutral-500 mt-1">{t("achieve.2.desc" as any)}</div>
              </div>
            </motion.div>
            ...
            <motion.div variants={fadeUp} className="p-5 bg-white/60 dark:bg-neutral-900/40 backdrop-blur-md border border-neutral-200/50 dark:border-neutral-800/50 rounded-2xl flex flex-col items-start gap-3">
              ...
              <div>
                <div className="font-semibold text-neutral-900 dark:text-neutral-100">{t("achieve.3.title" as any)}</div>
                <div className="text-xs leading-relaxed text-neutral-500 mt-1">{t("achieve.3.desc" as any)}</div>
              </div>
            </motion.div>
            ...
            <motion.div variants={fadeUp} className="p-5 bg-white/60 dark:bg-neutral-900/40 backdrop-blur-md border border-neutral-200/50 dark:border-neutral-800/50 rounded-2xl flex flex-col items-start gap-3">
              ...
              <div>
                <div className="font-semibold text-neutral-900 dark:text-neutral-100">{t("achieve.4.title" as any)}</div>
                <div className="text-xs leading-relaxed text-neutral-500 mt-1">{t("achieve.4.desc" as any)}</div>
              </div>
            </motion.div>
